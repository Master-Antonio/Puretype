    ...
    uint8_t* pBuffer)
{
    ...
    auto blendMask = [&](uint8_t* dstPx, float maskB, float maskG, float maskR)
    {
        ...
        const float outB = bgB * (1.0f - maskB) + linTextB * maskB;
        const float outG = bgG * (1.0f - maskG) + linTextG * maskG;
        const float outR = bgR * (1.0f - maskR) + linTextR * maskR;
                            ...
                            float* outLuma) -> bool
    {
        ...
        for (int row = 0; row < kSampleRows; ++row)
        {
            ...
            {
                const int x = left + static_cast<int>(((col + 0.5f) * static_cast<float>(right - left)) / kSampleCols);
                    ...
                    for (int gy = 0; gy < glyphH; ++gy)
                    {
                        ...
                        for (int gx = 0; gx < glyphW; ++gx)
                        {
                            ...
                            if (pureType)
                            {
                                ...
                                blendMask(dstPx, maskB, maskG, maskR);
                            }
                            else
                            {
                                ...
                                blendMask(dstPx, mask, mask, mask);
                            }
                        }
                    }
                }
