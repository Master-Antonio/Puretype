// =============================================================================
//  blender.cpp — Gamma-Correct Compositing Engine
//
//  Composites filtered glyph bitmaps onto the screen using physically accurate
//  sRGB ↔ Linear blending. The pipeline:
//
//  1. Read background pixel from DC/RenderTarget → sRGBToLinear()
//  2. Read coverage from filtered glyph bitmap → sRGBToLinear()
//  3. Blend in linear space:  result = bg*(1-cov) + textColor*cov
//  4. Convert back:  linearToSRGB(result) → write to DC
//
//  The gamma parameter is a fine-tuning exponent applied ON TOP of sRGB.
//  At gamma=1.0 (default), the pipeline is pure IEC 61966-2-1 sRGB.
// =============================================================================
...
// =============================================================================
//  BlitToHDC — GDI compositing path
// =============================================================================
                         ...
                         float gamma)
{
    ...
    // ── Decode text color to linear space ────────────────────────────────
    ...
    // Near-black text: force pure black for maximum contrast on OLED
    if (nearBlackText) {
        linTextR = 0.0f;
        linTextG = 0.0f;
        linTextB = 0.0f;
    ...
    // ── Create DIB for reading background ────────────────────────────────
    BITMAPINFO bmi = {};
    ...
    // ── Per-pixel compositing in linear space ────────────────────────────
    for (int row = 0; row < bitmap.height; ++row) {
        ...
        for (int col = 0; col < bitmap.width; ++col) {
            ...
            // ── Decode per-channel coverage from sRGB → linear ──────────
            float covB = sRGBToLinear(srcPx[0]);
            float covG = sRGBToLinear(srcPx[1]);
            float covR = sRGBToLinear(srcPx[2]);

            // ── Coverage processing: chroma reduction ────────────────────
            // Merge per-channel coverage toward a single luminance value
            // to reduce color fringing while preserving subpixel resolution.
            ...
            // ── Near-black text: maximize OLED contrast ──────────────────
            if (nearBlackText) {
                float stemBoost =
            ...
            // ── Convert linear → sRGB for output ─────────────────────────
    ...
    // Blit composited result back to the screen DC
...
// =============================================================================
//  BlitToD2DTarget — Direct2D compositing path
// =============================================================================
                               ...
                               float gamma)
{
    ...
    // ── Colorize coverage → premultiplied BGRA using sRGB pipeline ───────
    ...
    for (int row = 0; row < bitmap.height; ++row) {
        for (int col = 0; col < bitmap.width; ++col) {
            ...
            // Premultiplied color: coverage × text color in linear, then → sRGB
